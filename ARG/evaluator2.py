...
def run(model_name = 'model_1', project_dir='./data/gender_race_face/'):
    ...
    model_dir = project_dir + 'logs/model_1'
    restore_checkpoint = model_dir + 'model_iters_final.ckpt'
    log_dir = model_dir + 'eval/'
    ckptpath = '/home/pagand/PycharmProjects/RaceRec/final_model/data/gender_race_face/models/model_low_lr2_final'
    ...
    images, genders, races, ages, addresses = read_and_decode(data_file, [image_size, image_size], is_training=False)
    ...
    loss = tf.add_n(
        [loss_genders, loss_races, loss_ages] + tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES))
    ...
    accuracy3 = tf.reduce_mean(tf.to_float(tf.equal(tf.to_int32(predictions3), races)))
    ...
    tf.summary.scalar('Validation_Accuracy_race', accuracy3)
    ...
    with tf.Session() as sess:
        ...
        accuracies1,  accuracies2, accuracies3, loss_list = [], [], [], []

        for step in range(num_steps_per_epoch):

            start_time = time.time()
            l, acc1, acc2, acc3, curr_summary = sess.run([loss, accuracy1, accuracy2, accuracy3, summary_op],
                                                               {train_mode: False})
            time_elapsed = time.time() - start_time

            # Log some information
            logging.info('Step %s: gender Accuracy: %.4f race Accuracy: %.4f age Accuracy: %.4f loss: %.4f  (%.2f sec/step)',
                         step, acc1, acc2, acc3, l, time_elapsed)
            ...
            if step % 10 == 0:
                ...
                logging.info('Current age Accuracy: %s', acc3)
        ...
        average_acc3 = np.mean(accuracies3)
        ...
        print('Average age accuracy: ', average_acc3)
