...
def run(model_name, project_dir, initial_learning_rate, batch_size, num_epoch):
    ...
    final_checkpoint_file = model_dir + 'model_low_lr2' + '_final'
    ...
    images, genders, races, ages, addresses = read_and_decode(data_file, [image_size, image_size], batch_size=batch_size)

    # build the multitask model
    train_mode = tf.placeholder(tf.bool)

    logits_gender, logits_race, logits_age, end_points, variables_to_restore = build_model(images, train_mode)
    ...
    loss_ages = losses(logits_age, ages)

    # total loss with regularization
    loss = tf.add_n(
        [loss_genders, loss_races/5, loss_ages/10])
    ...
    accuracy3 = tf.reduce_mean(tf.cast(tf.nn.in_top_k(logits_age, ages, 1), tf.float32))

    global_step = slim.train.get_or_create_global_step()    # Define your exponentially decaying learning rate

    lr_high = tf.train.exponential_decay(         #### Added by the squad
    ...
    lr_low_low = tf.train.exponential_decay(
        learning_rate=initial_learning_rate / 10000,
        global_step=global_step,
        decay_steps=decay_steps,
        decay_rate=learning_rate_decay_factor,
        staircase=True)
    ...
    age_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='Age')

    gender_opt = tf.train.AdamOptimizer(learning_rate=lr_low_low)
    race_opt = tf.train.AdamOptimizer(learning_rate = lr_low_low)
    age_opt = tf.train.AdamOptimizer(learning_rate=lr_low)
    ...
    with tf.control_dependencies(update_ops):
        ...
        #train_age_op = slim.learning.create_train_op(loss_ages, age_opt, global_step, variables_to_train=age_vars)
        ...
        train_age_op = age_opt.minimize(loss_ages, var_list=age_vars, global_step = global_step)
        train_restnet_op = final_optimizer.minimize(loss, var_list=final_variables, global_step = global_step)

        train_op = tf.group(train_gender_op, train_race_op, train_age_op, train_restnet_op)
    ...
    tf.summary.scalar('accuracy_age', accuracy3)
    ...
    with tf.Session() as sess:
        ...
        if False:     ### Turn back to true after this first run!!!!!!!!!!!!*(************** very important
            #saver.restore(sess, ckpt.model_checkpoint_path)
            saver.restore(sess, final_checkpoint_file)
        ...
        for epoch in range(num_epoch):
            ...
            for step in range(num_steps_per_epoch): # num_steps_per_epoch
                ...
                if step % 100 == 0:
                    log1, log2, log3, l1, l2, l3, addr, learning_rate_value, accuracy_value1, accuracy_value2, accuracy_value3, l_gender, l_race, l_age = sess.run(
                        [logits_gender, logits_race, logits_age, genders, races, ages, addresses,
                         lr, accuracy1, accuracy2, accuracy3, loss_genders, loss_races, loss_ages],
                        {train_mode: True})

                    print ('loss for gender: ', l_gender)
                    print ('loss for race: ', l_race)
                    print('loss for age: ', l_age)
                    ...
                    logging.info('Current age Accuracy: %s', accuracy_value3)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--project_dir", type=str, default="/home/pagand/PycharmProjects/RaceRec/final_model/data/gender_race_face/", help="Path to project")
    parser.add_argument("--model_name", type=str, default="model_low_lr2_20", help="Model name")
